...
contract StakingReward is Owned, ReentrancyGuard, EIP712 {
    ...
    ) Owned(msg.sender) EIP712("SFK-StakingReward", "1"){
        USDT = IERC20(_usdt);
   ...
   function _hashPayoutArray(Payout[] calldata payouts) internal pure returns (bytes32) {
       bytes32[] memory hashes = new bytes32[](payouts.length);
       for (uint256 i = 0; i < payouts.length; i++) {
           hashes[i] = _hashPayout(payouts[i]);
       }
    ...
    function emitUnstakePerformanceUpdate(address user,uint256 amount) external onlyStaking {
        ...
        if(parent !=  address(0)){
            bool wasUnlocked = isUnlocked[user];
